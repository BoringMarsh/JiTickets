...
app.post('/api/payment', function(req, res, next) {
    ...
    // 支付成功或者失败打开的页面
    let returnURL = 'http://localhost:8080/UserInfoPage?id=' + user_id;
...
app.post('/api/withdraw', function(req, res, next) {
    ...
    const bizContent = {
        ...
        payeeInfo: {
            ...
            identityType: payee_info.identityType
        }
    };

    // 执行转账
    //alipaySdk.exec('alipay.fund.trans.uni.transfer', {
        //biz_content: JSON.stringify(bizContent) // This wraps all the transfer details into a single JSON string
    //}).then((response) => {
